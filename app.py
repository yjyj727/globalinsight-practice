...
# 기본 불용어 (문법어 + HTML 엔티티 잔재 + 뉴스 공통 노이즈 단어 통합)
BASE_STOPWORDS = {
    # 영어 문법어
    ...
    "get", "got", "been", "being", "our", "their", "which", "there",
    "were", "they", "then", "than", "your", "my", "its", "any", "may",
    # HTML 엔티티 잔재 토큰 — RSS description CDATA에서 남는 노이즈
    "nbsp", "amp", "quot", "apos", "lt", "gt",
    "ndash", "mdash", "laquo", "raquo", "ldquo", "rdquo",
    "hellip", "rsquo", "lsquo", "bull", "middot", "copy",
    # 뉴스 RSS 공통 노이즈 (날짜/숫자 표현 등)
    "com", "www", "http", "https", "utm", "via", "per",
}
...
def fetch_media_news(keyword: str, media_name: str) -> pd.DataFrame:
    ...
    for item in items[:50]:
        ...
        # description은 CDATA로 감싼 HTML일 수 있으므로 BeautifulSoup으로 완전 파싱
        import html as _html
        if desc_t:
            desc_raw = str(desc_t)
            # CDATA 내부 HTML을 파싱해 텍스트 추출
            desc_soup = BeautifulSoup(desc_raw, "html.parser")
            desc_r = desc_soup.get_text(separator=" ", strip=True)
            desc_r = _html.unescape(desc_r)           # 엔티티 변환
            desc_r = re.sub(r"&[a-zA-Z]{2,8};", " ", desc_r)  # 잔여 엔티티 제거
            desc_r = re.sub(r"\s+", " ", desc_r).strip()
        else:
            desc_r = ""
        desc   = desc_r[:400]
...
def extract_word_freq(texts: list, stopwords: set) -> Counter:
    ...
    - BeautifulSoup으로 HTML 태그 + 엔티티 완전 제거
    - 길이 3 미만 토큰 제거
    - stopwords 필터링
    """
    import html as _html
    ...
    for col_w, df_m, media_nm, cmap, hdr_cls, texts_m in [
        ...
        with col_w:
            ...
            if not texts_m:
                ...
                clean_parts = []
                for t in texts_m:
                    t2 = _html.unescape(str(t))
                    t2 = re.sub(r"&[a-zA-Z]{2,8};", " ", t2)
                    clean_parts.append(t2)
                raw_text = re.sub(r"[^a-zA-Z\s]", " ", " ".join(clean_parts))
                # 불용어 + 미디어명 모두 제거한 단어만 사용
