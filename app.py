...
# 기본 불용어 (문법어 + HTML 태그/속성명 잔재 + CSS값 + RSS 노이즈 통합)
BASE_STOPWORDS = {
    # 영어 문법어
    ...
    "where", "than", "so", "if", "all", "no", "say", "just", "can",
    "get", "got", "being", "our", "which", "there", "were", "then",
    "your", "my", "any", "may", "each", "between", "through", "these",
    "those", "such", "even", "back", "well", "now", "still", "last",
    "also", "while", "too", "very", "only", "much", "many", "most",
    # ── HTML 태그명 잔재 ────────────────────────────────────────────
    "html", "head", "body", "div", "span", "table", "tbody", "thead",
    "tfoot", "tr", "td", "th", "ul", "ol", "li", "nav", "header",
    "footer", "main", "section", "article", "aside", "form", "input",
    "button", "label", "select", "option", "img", "src", "alt",
    "script", "style", "link", "meta", "title", "iframe", "embed",
    "object", "param", "video", "audio", "source", "canvas", "svg",
    "path", "rect", "circle", "text", "tspan", "pre", "code",
    "blockquote", "figure", "figcaption", "strong", "small", "mark",
    "abbr", "cite", "time", "data",
    # ── HTML 속성명 잔재 ────────────────────────────────────────────
    "href", "src", "rel", "type", "name", "class", "target",
    "width", "height", "align", "valign", "border", "cellpadding",
    "cellspacing", "colspan", "rowspan", "bgcolor", "style",
    "onclick", "onload", "onmouseover", "onmouseout",
    # ── CSS 속성·값 잔재 ────────────────────────────────────────────
    "color", "font", "size", "face", "bold", "italic", "normal",
    "none", "block", "inline", "flex", "grid", "auto", "solid",
    "hidden", "visible", "scroll", "pointer", "cursor", "margin",
    "padding", "display", "position", "absolute", "relative",
    "background", "transparent", "inherit", "important",
    # ── HTML 엔티티 잔재 ────────────────────────────────────────────
    ...
    # ── RSS / URL 노이즈 ─────────────────────────────────────────────
    "rss", "xml", "feed", "atom", "google", "news", "articles",
    "article", "read", "more", "full", "story", "click", "here",
    "com", "www", "http", "https", "utm", "via", "per", "net", "org",
    "lxte", "cbmipgfbvv", "cbmisgfbvv", "ggraufvx", "aufvx",
    "cbmirafbvv", "cbminwfbvv", "ggaufvx", "ggyaufvx", "fvx",
    # ── 뉴스 공통 반복 단어 ──────────────────────────────────────────
...
def fetch_media_news(keyword: str, media_name: str) -> pd.DataFrame:
    ...
    for item in items[:50]:
        ...
        # ── description 정제 파이프라인 ──────────────────────────────
        # Google News RSS description은 <a href="...">, <font color="..."> 등
        # 내부 HTML이 CDATA로 감싸져 있어 태그 속성값(href URL, color값 등)까지
        # 텍스트로 추출되는 문제가 있음.
        # 해결: 정규식으로 태그를 통째로 제거한 뒤 엔티티 디코딩
        import html as _html
        if desc_t:
            # 원본 문자열에서 CDATA 안쪽 내용 추출
            raw = str(desc_t)
            # 1) <태그 ...> 전체를 공백으로 치환 (속성값 포함 제거)
            raw = re.sub(r"<[^>]+>", " ", raw)
            # 2) HTML 엔티티 디코딩 (&amp; → &, &nbsp; → 공백 등)
            raw = _html.unescape(raw)
            # 3) 디코딩 후에도 남은 &word; 패턴 제거
            raw = re.sub(r"&[a-zA-Z]{2,10};", " ", raw)
            # 4) URL 패턴 제거 (http://... https://... 및 www.)
...
def get_stopwords(keyword: str, media_names: list = None) -> set:
    """기본 불용어 + 검색 키워드 + 미디어명 단어를 모두 불용어에 추가
    - 미디어명(BBC, CNN 등)이 워드클라우드에 압도적으로 나타나는 것을 방지
    """
    # 검색 키워드 분해 (예: "climate change" → {"climate", "change"})
    extra = set(keyword.lower().split())

    # 미디어명 분해 (예: "NY Times" → {"ny", "times"}, "BBC" → {"bbc"})
    media_tokens = set()
    if media_names:
        for name in media_names:
            tokens = re.findall(r"[a-zA-Z]+", name.lower())
            media_tokens.update(tokens)
            # 도메인 이름도 추가 (예: bbc.com → "bbc", "com")
            domain = MEDIA_DOMAINS.get(name, "")
            domain_tokens = re.findall(r"[a-zA-Z]+", domain.lower())
            media_tokens.update(domain_tokens)
...
def extract_word_freq(texts: list, stopwords: set) -> Counter:
    ...
    - 정규식으로 HTML 태그(속성값 포함)를 통째로 제거
    - html.unescape로 엔티티 변환
    - URL 패턴 제거
    - 3글자 미만 토큰 제거
    - stopwords 필터링
    """
    ...
    for text in texts:
        t = str(text)
        # 1) HTML 태그 전체 제거 (href="...", color="..." 등 속성값 포함)
        t = re.sub(r"<[^>]+>", " ", t)
        # 2) 엔티티 디코딩
        t = _html.unescape(t)
        # 3) 잔여 엔티티 패턴 제거
...
# 9. 공통 계산 (모든 탭에서 재사용)
# ──────────────────────────────────────────────
# 키워드 + 선택된 미디어명(BBC, CNN 등)을 모두 불용어에 포함
    ...
    for col_w, df_m, media_nm, cmap, hdr_cls, texts_m in [
        ...
        with col_w:
            ...
            if not texts_m:
                ...
                clean_parts = []
                for t in texts_m:
                    t2 = re.sub(r"<[^>]+>", " ", str(t))   # HTML 태그(속성값 포함) 제거
                    t2 = _html.unescape(t2)                  # 엔티티 디코딩
                    t2 = re.sub(r"&[a-zA-Z]{2,10};", " ", t2)
                    t2 = re.sub(r"https?://\S+", " ", t2)   # URL 제거
                    t2 = re.sub(r"www\.\S+", " ", t2)
                    clean_parts.append(t2)
                raw_text = re.sub(r"[^a-zA-Z\s]", " ", " ".join(clean_parts))
